...
export function useAdminProductSetsListing() {
  ...
  const productSets = ref<AdminProductSet[]>([])
  ...
  async function loadProductSets() {
    ...
    const result = await getAdminProductSets({
    ...
    productSets.value = result.data.data
